...
function getContacts(filterBy = null) {
    return new Promise((resolve, reject) => {
        var contactsToReturn = JSON.parse(localStorage.contacts || null)
        if (!contactsToReturn) {
...
function transferTo(contactId, amount) {
    // TODO ask from server session
    return new Promise(async (resolve, reject) => {
        const loggedinUser = JSON.parse(localStorage.loggedinUser)
        if (!loggedinUser || loggedinUser.coins < amount) return reject()
        try {
            const contact = await getContactById(contactId)
            contact.coins = contact.coins ? contact.coins + amount : amount
